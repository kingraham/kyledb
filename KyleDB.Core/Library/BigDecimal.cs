...
namespace Common
{
    ...
    /// </summary>
    /// Modifications made to support decimal
        ...
        , IComparable<BigDecimal>
    {
        ...
        {
            int exponent = 0;
            while (dec - decimal.Truncate(dec) > 0)
            {
                dec *= 10;
                exponent++;
            }
            Setup(new BigInteger(dec), -exponent);
        }

        public BigDecimal(string str) : this()
        {
            var decimalIdx = str.IndexOf('.');
            if (decimalIdx < 0)
            {
                Setup(BigInteger.Parse(str), str.Length);
            } else
            {
                var exponent = str.Length - decimalIdx - 1;
                Setup(BigInteger.Parse(str.Remove(decimalIdx, 1)), -exponent);
            }                        
        }

        private void Setup(BigInteger mantissa, int exponent)
        {
            Mantissa = mantissa;
            Exponent = exponent;
            Normalize();
        ...
        #endregion Additional mathematical functions
